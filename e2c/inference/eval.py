...
from .tts.strategies import apply_tts_strategy
from .tts_prompts import EXPLORATION_PROMPT
...
def generate_explorations(
    ...
    exploration_prompts = []
    for q in questions:
        prompt = EXPLORATION_PROMPT.format(problem=q)
        exploration_prompts.append(prompt)
    input_ids = tokenizer(
        exploration_prompts,
        return_tensors="pt",
        padding=True,
    ...
    with torch.no_grad():
        outputs = model.generate(
            **input_ids,
            ...
            top_p=top_p,
            do_sample=True,
            num_return_sequences=k_explorations,
            eos_token_id=tokenizer.eos_token_id,
            pad_token_id=tokenizer.pad_token_id
        )
    all_plans = tokenizer.batch_decode(outputs[:, input_ids.input_ids.shape[1]:], skip_special_tokens=True)

    batched_plans = []
    for i in range(len(questions)):
        start_idx = i * k_explorations
        end_idx = start_idx + k_explorations
        question_plans = [plan.replace("<|im_end|>", "").strip() for plan in all_plans[start_idx:end_idx]]
        batched_plans.append(question_plans)
    return batched_plans
def weighted_vote_and_evaluate(
    ...
    for output, weight in zip(outputs, weights):
        answer = str(output.get("answer", "No Answer"))
    ...
    is_correct = False
    if isinstance(gt_answer, dict):  # 多选题
        if voted_answer == gt_answer.get('answer_idx'):
            is_correct = True
    else:
        norm_voted, _ = normalize_final_answer(voted_answer)
        norm_gt, _ = normalize_final_answer(gt_answer)
        if norm_voted == norm_gt:
            is_correct = True

    return is_correct, voted_answer


def evaluate_batch(
        questions: List[str],
        gt_answers: List[Any],
        samples_per_question: List[List[Tuple[List, List, List]]],
        cfg: Any,
        tokenizer: Any,
        weights_per_question: List[List[float]] = None
) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    K = len(questions)
    entropy_calculator = EntropyCalculator(tokenizer)
    results = []
    total_avg_success, total_best_success, total_format_avg = 0, 0, 0
    for i in range(K):
        current_samples = samples_per_question[i]
        current_weights = weights_per_question[i] if weights_per_question else [1.0] * len(current_samples)
        sample_num = len(current_samples)
        if sample_num == 0:
            results.append({
                "question": questions[i], "gt_answer": gt_answers[i],
                "samples": [], "failed_samples": [], "avg_success": 0.0, "best_success": 0,
                "finish_rate": 0, "finish_and_correct_rate": 0,
            })
            continue
        per_sample_success = []
        per_sample_format = []
        ...
        for s_idx in range(sample_num):
            tokens, entropies, confidences = current_samples[s_idx]
            pred_text, _ = decode_predictions(tokenizer, [tokens], cfg.eval.head + cfg.eval.tail, cfg.eval.answer_seg)
            full_text = pred_text[0]
            succ, pre_answer = (None, "")
            ...
            fmt = "boxed" in full_text
            per_sample_success.append(succ)
            per_sample_format.append(fmt)
            per_sample_outputs.append({
                "answer": pre_answer, "full_text": full_text, "success": succ, "format_success": fmt,
        ...
        weighted_success_sum = sum(float(s) * w for s, w in zip(per_sample_success, current_weights))
        total_weight = sum(current_weights)
        avg_success = weighted_success_sum / total_weight if total_weight > 0 else 0.0
        is_tts_cluster = cfg.eval.get('tts', {}).get('enabled', False) and cfg.eval.tts.strategy == 'semantic_cluster'
        if is_tts_cluster:
            best_success_bool, _ = weighted_vote_and_evaluate(per_sample_outputs, current_weights, gt_answers[i])
            best_success = 1 if best_success_bool else 0
        else:
            best_success = 1 if any(per_sample_success) else 0
        format_avg = sum(per_sample_format) / len(per_sample_format) if per_sample_format else 0.0
        total_avg_success += avg_success
        total_best_success += best_success
        total_format_avg += format_avg
        results.append({
            "question": questions[i], "gt_answer": gt_answers[i], "samples": per_sample_outputs,
            "avg_success": avg_success, "best_success": best_success,
            "finish_rate": sum(
                1 for out in per_sample_outputs if out['boxed_count'] > 0) / sample_num if sample_num > 0 else 0,
            "finish_and_correct_rate": sum(1 for out in per_sample_outputs if out['boxed_count'] > 0 and out[
                'success']) / sample_num if sample_num > 0 else 0,
        })
    batch_stats = {
        "avg_success": total_avg_success / K if K > 0 else 0,
        "best_success": total_best_success / K if K > 0 else 0,
        "format_avg": total_format_avg / K if K > 0 else 0,
        "all_number": K,
        "entropy_stats": {}
    }
...
def eval_main(cfg):
    ...
    overall_stats = {}
    dataset_list = cfg.eval.dataset
    if 'all' in dataset_list:
        dataset_list = ["gsm8k", "math", "aime24", "aime25", "amc23", "math500", "minerva", "olympiad_bench"]
    if 'med' in dataset_list:
        dataset_list = ["clinical_knowledge", "college_biology", "college_medicine", "medical_genetics",
                        "professional_medicine", "anatomy", "medqa", "medmcqa"]
    for dataset_name in dataset_list:
        dataset, dataset_name = load_dataset_by_name(dataset_name)
        is_tts_enabled = cfg.eval.get('tts', {}).get('enabled', False)
        batch_size = 1
        max_tokens = max_token_dataset.get(dataset_name,
                                           1000) if cfg.eval.max_new_tokens == -1 else cfg.eval.max_new_tokens
        save_path = os.path.join(cfg.eval.save_path, dataset_name)
        os.makedirs(save_path, exist_ok=True)

        results, static = (check_resume(save_path, cfg.eval.seed, rank) if cfg.eval.get('resume', False) else ([], {}))
        subset, subset_size = get_distributed_dataloader(dataset, batch_size, rank, world_size)

        bar = tqdm.tqdm(total=subset_size, desc=f"Processing {dataset_name} on Rank {rank}", position=rank)

        start_idx = len(results)
        bar.update(start_idx)
        for i in range(start_idx, subset_size):
            batch_data = subset[i:i + batch_size]
            raw_questions = [d["question"] for d in batch_data]
            gt_answers = [d["answer"] for d in batch_data]

            samples_for_eval = []
            weights_per_question = None

            # ---TTS---
            if is_tts_enabled:
                tts_cfg = cfg.eval.tts
                if rank == 0 and i == start_idx:
                    print(f"\n[TTS Mode Enabled] Strategy: {tts_cfg.strategy}, K={tts_cfg.k_explorations}")
                plans = generate_explorations(
                    model.module, tokenizer, raw_questions,
                    k_explorations=tts_cfg.k_explorations, device=device
                )[0]

                strategy_kwargs = {
                    "model": model.module, "tokenizer": tokenizer, "question": raw_questions[0], "device": device,
                    "num_clusters": tts_cfg.get("num_clusters", 3),
                    "embedding_model_name": tts_cfg.get("embedding_model", "all-mpnet-base-v2")
                }
                selected_plans, weights = apply_tts_strategy(tts_cfg.strategy, plans, **strategy_kwargs)
                weights_per_question = [weights]
                execution_prompts = []
                if selected_plans:
                    for plan in selected_plans:
                        prompt = tokenizer.apply_chat_template(
                            [{"role": "user",
                              "content": raw_questions[0] + cfg.eval.question_suffix.replace("<guideline>", plan)}],
                            tokenize=False, add_generation_prompt=True
                        ) + cfg.eval.solution_prefix.replace("<guideline>", plan)
                        execution_prompts.append(prompt)

                    generated_samples_list = generate_and_compute_entropy(
                        model.module, tokenizer, execution_prompts, max_tokens, device,
                        stop_token=cfg.eval.get("stop_token"), sample_num=1,
                        temperature=cfg.eval.temperature, top_p=cfg.eval.top_p,
                    )
                    num_executions = len(execution_prompts)
                    reformatted_samples = []
                    for sample_idx in range(num_executions):
                        reformatted_samples.append(
                            (generated_samples_list[0][0][sample_idx],  # tokens
                             generated_samples_list[0][1][sample_idx],  # entropies
                             generated_samples_list[0][2][sample_idx])  # confidences
                        )
                    samples_for_eval.append(reformatted_samples)
                else:
                ...
                prompts = [
                    tokenizer.apply_chat_template(
                        [{"role": "user", "content": q + cfg.eval.question_suffix}],
                        tokenize=False, add_generation_prompt=True
                    ) + cfg.eval.solution_prefix
                    for q in raw_questions
                ]
                generated_samples = generate_and_compute_entropy(
                    model.module, tokenizer, prompts, max_tokens, device,
                    stop_token=cfg.eval.get("stop_token"), sample_num=cfg.eval.sample_num,
                    temperature=cfg.eval.temperature, top_p=cfg.eval.top_p
                )
                samples_for_eval.append(
                    [(s[0][0], s[1][0], s[2][0]) for s in generated_samples]
                )

            batch_results, batch_stats = evaluate_batch(
                raw_questions, gt_answers, samples_for_eval, cfg, tokenizer, weights_per_question
            )
            results.extend(batch_results)
            if not static: static = {"avg_success": 0, "best_success": 0, "format_avg": 0, "all_number": 0}

            new_total = static["all_number"] + batch_stats["all_number"]
            if new_total > 0:
                static["avg_success"] = (static["avg_success"] * static["all_number"] + batch_stats["avg_success"] *
                                         batch_stats["all_number"]) / new_total
                static["best_success"] = (static["best_success"] * static["all_number"] + batch_stats["best_success"] *
                                          batch_stats["all_number"]) / new_total
                static["format_avg"] = (static["format_avg"] * static["all_number"] + batch_stats["format_avg"] *
                                        batch_stats["all_number"]) / new_total
                static["all_number"] = new_total
            ...
            bar.update(1)
            bar.set_postfix(avg_s=f"{static['avg_success']:.2%}", best_s=f"{static['best_success']:.2%}")
