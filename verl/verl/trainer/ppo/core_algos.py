...
class FixedKLController:
    ...
    def __init__(self, kl_coef,boost_ratio=1.0):
        self.value = kl_coef
...
def get_kl_controller(kl_ctrl):
    ...
    if kl_ctrl.type == "fixed":
        return FixedKLController(kl_coef=kl_ctrl.kl_coef,boost_ratio=kl_ctrl.get("boost_ratio",1.0))
...
def compute_rewards(token_level_scores, old_log_prob, ref_log_prob, kl_ratio):
    ...
    """
    # return compute_rewards_with_entropy(
    #     token_level_scores, old_log_prob, ref_log_prob, kl_ratio, entropy_ratio=0.01
    # )
    
    kl = old_log_prob - ref_log_prob
    return token_level_scores - kl * kl_ratio
def compute_rewards_with_entropy(
    token_level_scores,
    old_log_prob,
    ref_log_prob,
    kl_ratio,
    ...
    """
    计算带有 KL 惩罚和模长归一化几何衰减熵奖励的 token 级别奖励。

    Args:
        token_level_scores (torch.Tensor): Token 级别的基础奖励分数。
        old_log_prob (torch.Tensor): 当前策略的对数概率。
        ref_log_prob (torch.Tensor): 参考策略的对数概率。
        kl_ratio (float): KL 惩罚系数。
        entropy_ratio (float): 熵奖励的系数。
        k (float): 几何级数的衰减基数，应小于1。

    Returns:
        torch.Tensor: 最终的 token 级别奖励。
    """
    # 1. 计算 KL 惩罚
    kl_penalty = (old_log_prob - ref_log_prob) * kl_ratio

    # 2. 计算熵向量
    entropies = -old_log_prob * torch.exp(old_log_prob)

    # 3. 对熵向量进行模长归一化 (L2 normalization)
    # 确保模长不为0，避免除法错误
    norm = torch.norm(entropies, p=2, dim=-1, keepdim=True)
    l2_normalized_entropies = entropies / (norm + 1e-8)

    # 4. 生成几何级数向量 [k^0, k^1, k^2, ...]
    sequence_length = entropies.shape[-1]
    indices = torch.arange(sequence_length, device=entropies.device)
    geometric_progression = torch.pow(k, indices)
    
    # 广播 geometric_progression 以匹配批次大小
    geometric_progression = geometric_progression.expand_as(l2_normalized_entropies)

    # 5. 计算最终的熵奖励向量
    entropy_bonus = l2_normalized_entropies * geometric_progression * entropy_ratio
    
    
    final_rewards = token_level_scores - kl_penalty + entropy_bonus
...
@register_policy_loss("era")
def compute_policy_loss_era(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """
    Compute the ERA (Entropy Regularizing Activation) policy loss.
    
    This loss function applies ERA activation during the forward pass and uses
    the standard PPO loss computation with potentially modified advantages.
    ...
    Args:
        ...
        advantages (torch.Tensor):
            Advantage estimates for each action, shape (batch_size, response_length).
            These may have been modified by ERA activation.
        response_mask (torch.Tensor):
            Mask indicating which tokens to include in the loss, shape (batch_size, response_length).
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. Defaults to "token-mean".
        config (Optional[DictConfig | AlgoConfig], optional):
            Configuration object containing ERA and PPO parameters.
    """
    
    assert config is not None
    assert not isinstance(config, AlgoConfig)
    
    # Use the same PPO loss computation as vanilla
    ...
    assert clip_ratio_c > 1.0, (
        f"clip_ratio_c should be greater than 1.0, but got the value: {clip_ratio_c}."
    )
