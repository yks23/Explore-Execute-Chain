...
from process_checkpoint import process_checkpoint
...
class FSDPSFTTrainer:
    def __init__(
        ...
        if self.config.trainer.kl_coeff > 0:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # self._KL_loss(batch['reference'], do_backward=False)
            ...
            if self.config.trainer.get("kl_coeff", 0) > 0:
                KL_loss = self._KL_loss(batch, do_backward=True)
                loss = loss + self.config.trainer.kl_coeff * KL_loss
            return loss
    def _KL_loss(self, batch,do_backward=True):
        """Compute loss with optional sequence parallelism and remove padding features"""
        use_sp = self.use_remove_padding and self.config.ulysses_sequence_parallel_size > 1

        # Move inputs to GPU and prepare loss mask
        input_ids = batch["input_ids_ref"].to(self.device_name)
        attention_mask = batch["attention_mask_ref"].to(self.device_name)
        special_attention_mask = batch['special_attention_mask_ref'].to(self.device_name)
        position_ids = batch["position_ids_ref"].to(self.device_name)
        loss_mask = batch.pop("loss_mask_ref")[:, :-1].reshape(-1).to(self.device_name)

        # Use KLDivLoss instead of CrossEntropyLoss
        # Reduction is set to 'none' so we can apply the loss mask manually
        kl_loss_fct = nn.KLDivLoss(reduction="none")
            ...
            if not use_sp:
                ...
                with torch.no_grad():
                    ref_output = self.ref_model(
                        input_ids=input_ids, attention_mask=special_attention_mask, position_ids=position_ids, use_cache=False
                    )
                ref_logits = ref_output.logits

                # Prepare shifted logits for loss calculation
                shift_logits = logits[..., :-1, :].contiguous()
                shift_ref_logits = ref_logits[..., :-1, :].contiguous()
                
                # Flatten the tokens
                shift_logits = shift_logits.view(-1, self.model.config.vocab_size)
                shift_ref_logits = shift_ref_logits.view(-1, self.model.config.vocab_size)

                # Apply log-softmax to current model's logits and softmax to reference model's logits
                log_p = F.log_softmax(shift_logits, dim=-1)
                ref_p = F.softmax(shift_ref_logits, dim=-1)
                
                # Compute KL divergence
                loss = kl_loss_fct(log_p, ref_p)
                # Sum the loss across the vocabulary dimension for each token
                loss = loss.sum(dim=-1)
                # Apply the loss mask to select only the valid tokens
                loss = loss * loss_mask.to(loss.device)

            else:
                # Sequence parallelism forward pass
                batch_size, seqlen = input_ids.shape
                ...
                ref_logits_rmpad = ref_output.logits.squeeze(0)

                # Apply log-softmax to current model's logits and softmax to reference model's logits
                log_p = F.log_softmax(logits_rmpad, dim=-1)
                ref_p = F.softmax(ref_logits_rmpad, dim=-1)

                # Compute KL divergence and sum over vocab dimension
                loss = kl_loss_fct(log_p, ref_p).sum(dim=-1)

                # Gather and unpad for sequence parallelism
                loss = gather_outputs_and_unpad(loss, gather_dim=0, unpad_dim=0, padding_size=pad_size)

                # Pad back to the original full loss tensor
                full_loss = pad_input(
                    hidden_states=loss.unsqueeze(-1), indices=indices, batch=batch_size, seqlen=seqlen
                )
        ...
        # Normalize loss over valid tokens across all data parallel ranks
        valid_token_this_rank = torch.sum(loss_mask)
        ...
        loss = torch.sum(loss) / (valid_token_this_rank + 1e-8) * dp_size
        loss = loss*self.config.trainer.kl_coeff
        if do_backward:
            loss.backward()
        print("KL_loss",loss)
        return loss
    ...
    def _init_checkpoint_manager(self):
        ...
        # Convert to DictConfig for compatibilitynum
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step or (self.config.trainer.save_freq > 0 and is_save_step):
                    ...
                    if self.config.model.lora_rank!=-1 and rank==0:
                        process_checkpoint(
                            path=os.path.join(self.config.trainer.default_local_dir, f"global_step_{global_step}", "huggingface"),
                            device='cuda'
                        )
                        
                if self.config.trainer.test_freq > 0 and is_valid_step:
                    from eval_util  import get_all_success
                    all_results = get_all_success(
                        model=self.fsdp_model,
                    ...
                    # 把各个rank的结果都收集到rank0上
                    if is_cuda_available:
                        all_results_list = [None for _ in range(self.device_mesh.size(0))]
                        torch.distributed.gather_object(all_results, all_results_list if rank == 0 else None, dst=0)
                        if rank==0:
                            success_list = [result["avg_success"] for result in all_results_list if result is not None]
                            success_rate = sum(success_list) / len(success_list)
                            save_path = self.config.eval.get("save_path", None)
                            if save_path is not None:
                                os.makedirs(save_path, exist_ok=True)
                                with open(os.path.join(save_path, f"global_step_{global_step}_result.txt"), "w") as f:
                                    f.write(f"Success Rate: {success_rate}\n")
                                with open(os.path.join(save_path, f"global_step_{global_step}_all_results.json"), "w") as f:
                                    json.dump(all_results, f)
                    # cfg = self.config.eval
                    # cfg.model_path = os.path.join('/home/fit/alex/Kaisen.Yang/CoT Decomposition/verl/checkpoints',self.config.trainer.project_name,self.config.trainer.experiment_name, f"global_step_{global_step}",'huggingface')
                    # cfg.save_path = os.path.join('./evaluation',str(self.config.eval.dataset),'global_step_'+str(global_step))
                    # cfg.number = rank
                    # cfg.seed = rank
                    # cfg.batch_size = 16
                    # eval_main(cfg)
...
@hydra.main(config_path="config", config_name="sft_trainer_2", version_base=None)
def main(config):
    run_sft(config)
