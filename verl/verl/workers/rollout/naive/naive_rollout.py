...
from verl.utils.era_logits_processor import ERAConfigurableLogitsProcessor
...
class NaiveRollout(BaseRollout):
    def __init__(self, module: nn.Module, config):
        ...
        # Initialize ERA logits processor if enabled
        self.era_processor = None
        if hasattr(config, 'era') and config.era.get('enabled', False):
            self.era_processor = ERAConfigurableLogitsProcessor(config.era)
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        for _ in range(self.config.response_length):
            ...
            # Apply ERA processing if enabled
            if self.era_processor is not None:
                # Create dummy input_ids for ERA processor (only need shape)
                dummy_input_ids = torch.zeros(batch_size, 1, device=logits.device, dtype=torch.long)
                logits = self.era_processor(dummy_input_ids, logits)
