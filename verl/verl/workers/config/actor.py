...
__all__ = ["PolicyLossConfig", "ERAConfig", "ActorConfig", "FSDPActorConfig", "McoreActorConfig"]
...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options: 'vanilla', 'clip-cov', 'kl-cov', 'gpg', 'era'.
...
class ERAConfig(BaseConfig):
    """Configuration for ERA (Entropy Regularizing Activation).

    Args:
        enabled (bool): Whether to enable ERA activation.
        era_lb (float): Lower bound for entropy threshold. When entropy < era_lb, logits are scaled up.
        era_ub (float): Upper bound for entropy threshold. When entropy > era_ub, logits are scaled down.
        era_k (float): Scaling factor for logits modification.
        adv_scale (bool): Whether to apply inverse scaling to advantages for stability.
        topk_ratio (float): Ratio of top-k tokens to consider for entropy calculation (default: 0.2).
    """
...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        log_part_entropy (bool): Whether to log entropy for exploration/execution parts separately.
        ...
        adv_coeff (float): Advantage coefficient for exploration phase.
        special_token_1 (int): Token ID marking exploration-execution boundary (e.g., </EXPLORATION>).
        special_token_2 (int): Alternative token ID for boundary (e.g., <EXECUTION>).
        entropy_mask_coef_after (float): Entropy mask coefficient after special token (execution phase).
        entropy_mask_coef_before (float): Entropy mask coefficient before special token (exploration phase).
        entropy_mask_min_position (int): Minimum position for entropy masking.
        kl_mask_coef_after (float): KL mask coefficient after special token.
        kl_mask_coef_before (float): KL mask coefficient before special token.
        ...
        era (ERAConfig): Configuration for ERA (Entropy Regularizing Activation).
    """
    ...
    adv_coeff: float = 1.0
    special_token_1: int = 151672
    special_token_2: int = 151673
    entropy_mask_coef_after: float = 1.0
    entropy_mask_coef_before: float = 1.0
    entropy_mask_min_position: int = 5
    kl_mask_coef_after: float = 1.0
    kl_mask_coef_before: float = 1.0
